...
pub async fn check_auth_status_command(app_handle: tauri::AppHandle) -> Result<bool, String> {
    ...
    if auth_service::validate_token(&access_token).await {
        ...
        crate::services::local_log_service::write_local_log(&app_handle, "INFO", "Token berhasil digunakan.");
        return Ok(true); 
    }
    ...
    match auth_service::refresh_access_token(&refresh_token).await {
        Ok((new_access, new_refresh)) => {
            ...
            crate::services::local_log_service::write_local_log(&app_handle, "INFO", "System berhasil melakukan auto-refresh token.");
...
pub async fn logout_command(app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    crate::services::local_log_service::write_local_log(&app_handle, "INFO", "User berhasil melakukan logout dari perangkat.");
